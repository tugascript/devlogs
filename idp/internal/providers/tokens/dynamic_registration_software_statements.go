...
type SoftwareStatementClaims struct {
	RawMetadata                  map[string]json.RawMessage `json:"-"`
	...
	DefaultMaxAge                int64                      `json:"default_max_age,omitempty" validate:"omitempty,min=0,max=2147483647"`
...
func (t *Tokens) VerifySoftwareStatement(
	...
	}, jwt.WithValidMethods([]string{"RS256", "ES256", "EdDSA"}), jwt.WithIssuedAt()); err != nil {
	...
	payload, err := base64.RawURLEncoding.DecodeString(strings.Split(opts.SoftwareStatement, ".")[1])
	if err != nil {
		return SoftwareStatementClaims{}, jwt.RegisteredClaims{}, err
	}
	if err := json.Unmarshal(payload, &claims.RawMetadata); err != nil {
		return SoftwareStatementClaims{}, jwt.RegisteredClaims{}, err
	}
