...
<head>
    ...
    <title>{{.Status}} {{.ErrorCode}} - Dev Logs</title>
</head>
<body>
<div id="page-content">
...
<head>
    ...
    <title>500 Internal Server Error - Dev Logs</title>
</head>
<body>
<div id="page-content">
    <div id="title-container">
...
type ErrorTemplateOptions struct {
	Status       int
	ErrorCode    string
	MessageTitle string
	Messages     []string
...
func BuildErrorTemplate(options ErrorTemplateOptions) (string, error) {
	messageParagraphs := utils.MapSlice(options.Messages, func(msg *string) string {
		return fmt.Sprintf("<p>%s</p>", *msg)
	})
	errTemplate := fmt.Sprintf(errorTemplate, strings.Join(messageParagraphs, "\n"))
	...
	t, err := template.New(errorTemplateName).Parse(errTemplate)
	if err != nil {
		return "", err
	}

	var errorContent bytes.Buffer
	if err := t.Execute(&errorContent, data); err != nil {
		return "", err
	}

	return errorContent.String(), nil
}
